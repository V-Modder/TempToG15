...
BOOL CColorAndMonoDlg::OnInitDialog()
{
	...
	this->m_hInstLibrary=LoadLibrary(L"HidFTDll.dll");
	if (m_hInstLibrary == NULL)
	{
        FreeLibrary(m_hInstLibrary);
		MessageBox(L"LoadLibrary Failed!");
		exit(1);
	}

	_EMyDetectDevice = (EMyDetectDevice)GetProcAddress(m_hInstLibrary, "EMyDetectDevice");
	_EMyCloseDevice = (EMyCloseDevice)GetProcAddress(m_hInstLibrary, "EMyCloseDevice");
	//	EMySetCurrentDev	_EMySetCurrentDev;
	_EMySetCurrentDev = (EMySetCurrentDev)GetProcAddress(m_hInstLibrary, "EMySetCurrentDev");
	_EMyInitConfig=(EMyInitConfig)GetProcAddress(m_hInstLibrary,"EMyInitConfig");
	_EMyReadTemp=(EMyReadTemp)GetProcAddress(this->m_hInstLibrary,"EMyReadTemp");
	if ((_EMyDetectDevice == NULL) || (_EMyCloseDevice == NULL) )
	{
        FreeLibrary(m_hInstLibrary);
		MessageBox(L"GetProcAddress Failed!");
		exit(1);
	}
	int nDevCount = _EMyDetectDevice(m_hWnd);
	_EMySetCurrentDev(0);
	_EMyInitConfig(true);
...
void CColorAndMonoDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	CString s;
	double temp =	this->_EMyReadTemp(true);  
	wstringstream wss;
	if (temp==-100)
	{
		s = L"No Sensor";
    ...
    m_lcd.SetText(m_smallText1, s);
	CString cc = wu.conditionW().c_str();
	m_lcd.SetText(m_smallText2, cc);
    m_lcd.Update();
...
VOID CColorAndMonoDlg::InitLCDObjectsMonochrome()
{
	...
	HBITMAP logoBitmap_ = (HBITMAP)LoadImage( AfxGetInstanceHandle() , 
												L"C:\\Users\\V-Modder\\Desktop\\TempToG15\\Samples\\ColorAndMono\\background.bmp", 
												IMAGE_BITMAP, 
												160, 
												43, 
												LR_LOADFROMFILE); 
    m_logo = m_lcd.AddBitmap(160, 43);
    ...
    m_smallText1 = m_lcd.AddText(LG_STATIC_TEXT, LG_BIG, DT_CENTER, 70);
	...
	*/
    m_smallText2 = m_lcd.AddText(LG_STATIC_TEXT, LG_BIG, DT_LEFT, 80);
    m_lcd.SetOrigin(m_smallText2, 80, 1);
